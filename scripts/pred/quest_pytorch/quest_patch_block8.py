...
def quest_vllm_forward(
        ...
        def decode_quest_func(query,key_cache,value_cache,block_table,seq_lens):
            ...
            key = key.reshape(key.shape[0]*key.shape[1],key.shape[2],key.shape[3])[:seq_lens]
            value = value.reshape(value.shape[0]*value.shape[1],value.shape[2],value.shape[3])[:seq_lens]
            ...
            padding = padding_key.expand(-1, -1,padding_length,-1)
            pad_key = torch.cat([key, padding], dim=2)
            pad_value = torch.cat(
                [
                    value,
                    torch.zeros(
                        (value.shape[0], value.shape[1], padding_length, value.shape[3]),
                        device=value.device,
                        dtype=value.dtype
                    )
                ],
            ...
            chunk_pad_key = pad_key.reshape(pad_key.shape[0],pad_key.shape[1],pad_key.shape[2] // chunk_size, chunk_size, pad_key.shape[3])
            ...
            attn_output = sparse_attention_fixed_mask(query,pad_key,mask,pad_value).to(query.dtype)
        ...
        query = query.view(-1, self.num_heads, self.head_size) # (512,32,128)
        key = key.view(-1, self.num_kv_heads, self.head_size) # (512,8,128)
        value = value.view(-1, self.num_kv_heads, self.head_size) # (512,8,128)

        if kv_cache is not None: # (2,28096,16,8,128)
        ...
        if decode_meta := attn_metadata.decode_metadata:
            ...
            if layer_idx < 2:
                decode_output = torch.ops.vllm.flash_attn_with_kvcache(
                    decode_query.unsqueeze(1), # (1,1,32,128)
                    key_cache, # (28096,16,8,128)
                    value_cache, 
                    block_table=decode_meta.block_tables, # (1,8146)
                    cache_seqlens=decode_meta.seq_lens_tensor, #torch.size([1])
                    softmax_scale=self.scale, # 0.08838834764831845
                    causal=True,
                    alibi_slopes=self.alibi_slopes, # ''
                    softcap=self.logits_soft_cap, # 0
                ).squeeze(1) # decode_output.shape [1,32,128]
            else:
