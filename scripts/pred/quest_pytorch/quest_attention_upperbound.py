...
def forward(
    ...
    **kwargs,
    ...
    hidden_dim = key_states.shape[-1]
    
    attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)

    _, indices = attn_weights.topk(k=int(seq_length * self.topk_ratio), dim=-1)
    indices = indices.squeeze(2)
    indices_expanded = indices.unsqueeze(-1).expand(-1, -1, -1, hidden_dim)
    selected_k = torch.gather(key_states, dim=2, index=indices_expanded)
    selected_v = torch.gather(value_states, dim=2, index=indices_expanded)
    attention_scores = torch.matmul(query_states.float(), selected_k.transpose(-2, -1).float()) / math.sqrt(hidden_dim)
    attention_weights = torch.softmax(attention_scores, dim=-1)
    attn_output = torch.matmul(attention_weights, selected_v.float()).to(query_states.dtype)

    # attn_output = sparse_attention_fixed_mask(query_states,key_states,mask,value_states).to(query_states.dtype)

    # device = chunk_pad_key.device
    # total_size = pad_key.shape[-2]

    # if self.quest == True and need_estimate == True:
