...
ROOT_DIR="/remote-home/pengyichen/RULER_pyc/result_base_front" # the path that stores generated task samples and model predictions.
...
# Model and Tokenizer
source config_models_quest_base.sh
MODEL_NAME=${1}
...
for MAX_SEQ_LENGTH in "${SEQ_LENGTHS[@]}"; do
    ...
    for TASK in "${TASKS[@]}"; do
        CUDA_VISIBLE_DEVICES=4 python data/prepare.py \
            --save_dir ${DATA_DIR} \
            --benchmark ${BENCHMARK} \
            --task ${TASK} \
        ...
        for BUDGET_SIZE in "${BUDGET_SIZE[@]}"; do
            for CHUNK_SIZE in "${CHUNK_SIZE[@]}"; do
                SAVE_DIR="${PRED_DIR}/${BUDGET_SIZE}-${CHUNK_SIZE}-${BASE}"
                mkdir -p ${SAVE_DIR}
                start_time=$(date +%s)
                CUDA_VISIBLE_DEVICES=4 python pred/call_api_serial.py \
                    --data_dir ${DATA_DIR} \
                    --save_dir ${SAVE_DIR} \
                    --benchmark ${BENCHMARK} \
                    --task ${TASK} \
    ...
    CUDA_VISIBLE_DEVICES=4 python eval/evaluate.py \
        --data_dir ${PRED_DIR} \
        --benchmark ${BENCHMARK}
done
